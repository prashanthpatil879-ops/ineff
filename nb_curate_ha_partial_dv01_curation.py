...
dataset_key = "ha_partial_dv01_curation"
...
# PROGRAM_CODE + LEGAL_ID + SECURITY_CURRENCY.
total_dv01_df = (
    dv01_unpivot_df
...
fx_date_expr = (
    ...
    else F.to_date(F.col(fx_date_col), fx_date_fmt)
)
...
# A CAD-based rates file has no row for CAD itself, so the reporting currency is given a rate of 1 explicitly.
...
base_rows = dv01_with_fx_df.filter(F.col("SECURITY_CURRENCY") == F.lit(BASE_CURRENCY)).count()
logger.info(f"{base_rows} row(s) in the reporting currency {BASE_CURRENCY} had no FX row; applying FX_RATE = 1.0")
